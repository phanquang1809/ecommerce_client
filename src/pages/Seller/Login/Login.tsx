...
import { createUser, sendVerificationCode, verifyEmail } from "@/services/authServices";
import { Navigate, useLocation, useNavigate } from "react-router-dom";
import useUserStore from "@/store/userStore";
import { useTheme } from "@/components/theme-provider";
import LoginForm from "@/pages/Website/Auth/LoginForm";
import VerificationForm from "@/pages/Website/Auth/VerificationForm";
import CreateAccount from "@/pages/Website/Auth/CreateAccount";
import { SendEmailResetPassword } from "@/pages/Website/Auth/SendEmailResetPassword";
...
export default function Login() {
  ...
  const { login,isAuthenticated,user,setUser,setInitialized } = useUserStore();
  ...
  const handleLogin = async () => {
    ...
    if (!newErrors.userInput && !newErrors.password) {
      setIsLoading(true);
      const { status, message } = await login(userInput, password, rememberMe,true);
      setIsLoading(false);
      if (status === "success") {
        handleReset();
        navigate("/seller");
  ...
  const handleVerifyEmail = async () => {
    ...
    setIsLoading(true);
    const result = await verifyEmail(userInput, code,true);
    setIsLoading(false);
    if (result.status === "success") {
      ...
      if(result.token)
      {
        ...
        setStep("createAccount");
        navigate("/seller/create?token=" + result.token + "&email=" + userInput);
      }
      else
      {
        navigate("/seller");
  ...
  const handleSendEmail = async () => {
    if (!userInput.trim()) {
      setError({ ...error, userInput: "Vui lòng nhập email" });
      return;
    }
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!emailRegex.test(userInput)) {
      setError({ ...error, userInput: "Email không hợp lệ!" });
      return;
    }
    setIsLoading(true);
    try {
      const result = await sendVerificationCode(userInput,true);
      if (result.status === "success") {
        toast.success(
          <span className="text-xs">Vui lòng kiểm tra email của bạn 📩</span>
        );
        navigate("/seller/verify?email=" + userInput);
        handleReset();
      } else if (result.status === "error") {
        setError({ ...error, userInput: result.message || "Có lỗi xảy ra!" });
  ...
  const handleCreacteAccount = async () => {
      ...
      if (!newErrors.password && !newErrors.confirmPassword) {
        setIsLoading(true);
        const result = await createUser(userInput, password, confirmPassword,token);
        setIsLoading(false);
        if (result.status === "success") {
          handleReset();
          navigate("/seller");
  ...
  const handleSendVerificationCode = async () => {
    toast.promise(sendVerificationCode(userInput,true), {
      loading: (
        <span className="text-xs">Đang gửi email đến {userInput}...</span>
      ),
      success: (
        <span className="text-xs">Vui lòng kiểm tra email của bạn 📩</span>
      ),
  ...
  return (
    <div>
      <GradientBackground />
      <div className="text-container z-[10] w-full h-full absolute top-0 left-0 flex justify-center items-center text-white select-none drop-shadow-[1px_1px_rgba(0,0,0,0.1)]">
        <div className="flex gap-20 max-w-6xl mx-auto flex-col md:flex-row">
          ...
          <div className="max-w-[450px] relative w-[calc(100vw-32px)] mx-auto border rounded-lg p-6 flex flex-col bg-background">
            <img
              ...
              >
                {isLoading ? (
                  ...
                  <>
                    ...
                    {step === "signup" && "Đăng ký tài trở thành người bán"}
                    {step === "createAccount" && "Tạo tài khoản"}
                  </>
                )}
              </Button>
